...
import allreduce  # 确保 allreduce 模块在你的环境中可用
import batch_allreduce  # 确保 batch_allreduce 模块在你的环境中可用
import variable_mgr_util  # 确保 variable_mgr_util 模块在你的环境中可用
...
class VariableMgr(object):
    ...
    def savable_variables(self):
        """Returns a list/dict of savable variables to pass to tf.compat.v1.train.Saver."""
    ...
    def get_gradients_to_apply(self, device_num, gradient_state):
        ...
        if self.benchmark_cnn.enable_auto_loss_scale and device_num == 0:
            ...
            for grad, _ in tower_grad:
                has_inf_nan_list.append(tf.reduce_all(tf.math.is_finite(grad)))
                ...
                for tower_grads in reduced_grads:
                    with tf.compat.v1.colocate_with(tower_grads[0]):
                        ...
                        is_finite_list.append(tf.reduce_all(
                            [tf.reduce_all(tf.math.is_finite(g)) for g in tower_grads]))
...
class VariableMgrDistributedFetchFromPS(VariableMgr):
    ...
    def get_devices(self):
        ps_strategy = tf.compat.v1.training.GreedyLoadBalancingStrategy(
            self.benchmark_cnn.num_ps, tf.compat.v1.training.byte_size_load_fn)
        return [
            tf.compat.v1.train.replica_device_setter(
                worker_device=d,

        ...
        if depth == in_size:
            if stride == 1:
                ...
                shortcut = tf.keras.layers.AveragePooling2D(
                    pool_size=(1, 1), strides=(stride, stride))(input_layer)
        else:
            shortcut = tf.keras.layers.Conv2D(
                filters=depth, kernel_size=(1, 1), strides=(stride, stride),
                padding='same', use_bias=False)(input_layer)
            shortcut = tf.keras.layers.BatchNormalization()(shortcut)

        res = tf.keras.layers.Conv2D(
            filters=depth_bottleneck, kernel_size=(1, 1), strides=(stride, stride),
            padding='same', use_bias=False)(input_layer)
        res = tf.keras.layers.BatchNormalization()(res)
        res = tf.keras.layers.ReLU()(res)

        res = tf.keras.layers.Conv2D(
            filters=depth_bottleneck, kernel_size=(3, 3), strides=(1, 1),
            padding='same', use_bias=False)(res)
        res = tf.keras.layers.BatchNormalization()(res)
        res = tf.keras.layers.ReLU()(res)

        res = tf.keras.layers.Conv2D(
            filters=depth, kernel_size=(1, 1), strides=(1, 1),
            padding='same', use_bias=False)(res)
        res = tf.keras.layers.BatchNormalization()(res)

        output = tf.keras.layers.ReLU()(shortcut + res)
        cnn.top_layer = output
        print(f"cnn.top_layer.shape: {cnn.top_layer.get_shape().as_list()}")
        cnn.top_size = depth
...
def bottleneck_block_v1_5(cnn, depth, depth_bottleneck, stride):
    ...
    with tf.compat.v1.variable_scope(name):
        ...
        res = tf.keras.layers.Conv2D(
            filters=depth_bottleneck, kernel_size=(1, 1), strides=(1, 1),
            padding='same', use_bias=False)(input_layer)
        res = tf.keras.layers.BatchNormalization()(res)
        res = tf.keras.layers.ReLU()(res)

        res = tf.keras.layers.Conv2D(
            filters=depth_bottleneck, kernel_size=(3, 3), strides=(stride, stride),
            padding='same', use_bias=False)(res)
        res = tf.keras.layers.BatchNormalization()(res)
        res = tf.keras.layers.ReLU()(res)
...
def bottleneck_block_v2(cnn, depth, depth_bottleneck, stride):
    ...
    preact = tf.keras.layers.BatchNormalization()(input_layer)
    ...
    with tf.compat.v1.variable_scope(name):
        if depth == in_size:
            ...
            shortcut = tf.keras.layers.Conv2D(
                filters=depth, kernel_size=(1, 1), strides=(stride, stride),
                padding='same', use_bias=False)(preact)
            shortcut = tf.keras.layers.BatchNormalization()(shortcut)
...
def residual_block(cnn, depth, stride, version, projection_shortcut=False):
    ...
    if pre_activation:
        res = tf.keras.layers.BatchNormalization()(input_layer)
        res = tf.keras.layers.ReLU()(res)
    else:
        res = input_layer

    res = tf.keras.layers.Conv2D(
        filters=depth, kernel_size=(3, 3), strides=(stride, stride),
        padding='same', use_bias=False)(res)
    res = tf.keras.layers.BatchNormalization()(res)
    res = tf.keras.layers.ReLU()(res)
    ...
    if pre_activation:
        res = tf.keras.layers.Conv2D(
            filters=depth, kernel_size=(3, 3), strides=(1, 1),
            padding='same', activation=None, use_bias=False)(res)
        output = shortcut + res
    else:
        res = tf.keras.layers.Conv2D(
            filters=depth, kernel_size=(3, 3), strides=(1, 1),
...
class ResnetModel(model_lib.CNNModel):
    ...
    def add_inference(self, cnn):
        ...
        if self.version == 'v2':
            cnn.batch_norm()
            cnn.top_layer = tf.keras.layers.ReLU()(cnn.top_layer)
        print(f"shape: cnn.top_layer 11: {cnn.top_layer.get_shape().as_list()}")
        cnn.spatial_mean()
    ...
    def get_learning_rate(self, global_step, batch_size):
        ...
        return tf.cond(global_step < warmup_steps, lambda: warmup_lr, lambda: lr(global_step))
...
class ResnetCifar10Model(model_lib.CNNModel):
    ...
    def get_learning_rate(self, global_step, batch_size):
        ...
        return tf.keras.optimizers.schedules.PiecewiseConstantDecay(boundaries, values)(global_step)

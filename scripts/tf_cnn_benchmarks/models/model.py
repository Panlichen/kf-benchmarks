...
import convnet_builder  # 确保 convnet_builder 模块在你的环境中可用

# BuildNetworkResult encapsulates the result (e.g., logits) of a
...
class CNNModel(Model):
    ...
    def add_backbone_saver(self):
        """Creates a tf.compat.v1.train.Saver as self.backbone_saver for loading backbone.

        A tf.compat.v1.train.Saver must be created and saved in self.backbone_saver before
        ...
        """
        raise NotImplementedError(self.get_model_name() + ' does not have backbone model.')
    ...
    def get_synthetic_inputs(self, input_name, nclass):
        ...
        # inputs = tf.compat.v1.get_local_variable(inputs, name=input_name)
        inputs = tf.identity(inputs, name=input_name)  # 使用 tf.identity 设置名称
    ...
    def build_network(self,
        ...
        print(f"shapes: network.top_layer 1: {network.top_layer.get_shape().as_list()}")
        with tf.compat.v1.variable_scope('cg', custom_getter=network.get_custom_getter()):
            print(f"shapes: network.top_layer 2: {network.top_layer.get_shape().as_list()}")
            self.add_inference(network)
            print(f"shapes: network.top_layer 3: {network.top_layer.get_shape().as_list()}")
    ...
    def accuracy_function(self, inputs, logits):
        ...
        top_1_op = tf.reduce_sum(
            tf.cast(tf.nn.in_top_k(predictions=logits, targets=labels, k=1), self.data_type))
        top_5_op = tf.reduce_sum(
            tf.cast(tf.nn.in_top_k(predictions=logits, targets=labels, k=5), self.data_type))

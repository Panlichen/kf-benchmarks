...
flags.DEFINE_integer(
    ...
    None, 'Every n steps during training, pause training, run '
...
flags.DEFINE_string('piecewise_partial_exchange_schedule', '',
...
flags.DEFINE_integer(
    ...
    'producer.')
flags.DEFINE_integer(
    'num_batches', None, 'number of batches to run, excluding '
...
flags.DEFINE_float(
    'num_epochs', None, 'number of epochs to run, excluding warmup. '
    'This and --num_batches cannot both be specified.')
flags.DEFINE_float(
    'num_eval_epochs', None, 'number of eval epochs to run, excluding warmup. '
    'Defaults to --num_epochs')
flags.DEFINE_float(
    'stop_at_top_1_accuracy', None,
...
flags.DEFINE_boolean(
    'collect_eval_results_async', False,
...
flags.DEFINE_float(
    'display_perf_ewma',
    None, 'If set, display numbers of images/sec using exponentially '
    ...
    upper_bound=1)
flags.DEFINE_string(
    'data_dir', None, 'Path to dataset in TFRecord format (aka Example '
...
flags.DEFINE_integer(
    ...
    lower_bound=0)
flags.DEFINE_boolean(
    'compact_gradient_transfer', True, 'Compact gradient'
    'as much as possible for cross-device transfer and '
    'aggregation.')
flags.DEFINE_enum(
    'variable_consistency', 'strong', ('strong', 'relaxed'),
...
flags.DEFINE_enum(
    'local_parameter_device', 'gpu', ('cpu', 'gpu', 'CPU', 'GPU'),
...
flags.DEFINE_string(
    'graph_file', None, 'Write the model\'s graph definition to this file. '
...
flags.DEFINE_float(
    ...
    'be set.')
flags.DEFINE_float(
    'resnet_base_lr', None, "Base learning rate at bs=256. Only "
...
flags.DEFINE_float(
    ...
    0, 'If non-zero, the fraction of GPU memory that will be used. '
...
flags.DEFINE_integer(
    'gpu_pending_cap', 0, 'If > 0 then then number of pending '
...
flags.DEFINE_boolean(
    ...
    'step.')
flags.DEFINE_boolean(
    'staged_vars', False, 'whether the variables are staged from the main '
    'computation')
...
flags.DEFINE_string(
    'rewriter_config', None, 'Config for graph optimizers, described as a '
    'RewriterConfig proto buffer.')
flags.DEFINE_enum(
    'loss_type_to_report', 'total_loss', ('base_loss', 'total_loss'),
...
flags.DEFINE_boolean(
    'fp16_vars', False, 'If fp16 is enabled, also use fp16 for variables. If '
...
flags.DEFINE_integer(
    'allreduce_merge_scope', 1, 'Establish a name scope around this many '
...
flags.DEFINE_string(
    'horovod_device', '', 'Device to do Horovod all-reduce on: '
...
flags.DEFINE_integer(
    'summary_verbosity', 0, 'Verbosity level for summary ops. '
...
flags.DEFINE_integer(
    'trt_max_workspace_size_bytes', 4 << 30,
...
def create_config_proto(params):
  ...
  """
    ...
    if params.variable_update == 'collective_all_reduce':
        ...
        config.device_filters.append('/job:%s/replica:0/task:%d' %
                                     (params.job_name, params.task_index))
  ...
  """
    ...
    if params.eval:
        ...
        return constants.BenchmarkMode.FORWARD_ONLY
          ...
          or params.eval_during_training_every_n_epochs
          or params.eval_during_training_at_specified_steps
          or params.eval_during_training_at_specified_epochs):
...
def benchmark_one_step(sess,
    ...
    need_options_and_metadata = (
        should_profile or collective_graph_key > 0
        or ((trace_filename or partitioned_graph_file_prefix) and step == -2))
    ...
    # Display statistics
    if (show_images_per_sec and step >= 0
            and (step == 0 or (step + 1) % params.display_every == 0)):
        ...
        log_str = '[%10.3f]\t%6i\t%7i\t%s\t%.*f' % (
            sum(step_train_times), delta, step + 1,
        ...
        if benchmark_logger:
            benchmark_logger.log_metric('current_examples_per_sec',
                                        speed_mean,
                                        global_step=step + 1)
            if 'top_1_accuracy' in results:
                benchmark_logger.log_metric('top_1_accuracy',
                                            results['top_1_accuracy'],
                                            global_step=step + 1)
                benchmark_logger.log_metric('top_5_accuracy',
    ...
    if need_options_and_metadata:
        ...
        if partitioned_graph_file_prefix and step == -2:
            ...
            for graph_def in run_metadata.partition_graphs:
                device = graph_def.node[0].device.replace('/', '_').replace(
                    ':', '_')
    ...
    if params.checkpoint_every_n_epochs:
        ...
        if (step >= 0
                and (step == 0 or (step + 1) % checkpoint_interval == 0)):
            ...
            if ((step + 1) % params.display_every != 0):
                ...
                log_str = '[%10.3f]\t%6i\t%7i\t%s\t%.*f' % (
                    sum(step_train_times), delta, step + 1,
                    get_perf_timing_str(speed_mean, speed_uncertainty,
                                        speed_jitter),
                    LOSS_AND_ACCURACY_DIGITS_TO_SHOW, lossval)
                if 'top_1_accuracy' in results:
                    log_str += '\t%.*f\t%.*f' % (
...
def get_perf_timing(batch_size, step_train_times, ewma_alpha=None, scale=1):
    ...
    if ewma_alpha:
        weights = np.logspace(len(times) - 1,
                              0,
                              len(times),
                              base=1 - ewma_alpha)
...
def _get_checkpoint_to_load(ckpt_dir):
  ...
  """
    ...
    if os.getenv('KUNGFU_TEST_SELF_RANK') is None:
        print("Independent process. Worker rank not specified")
    else:
        ckp_dir = ckpt_dir + "-worker-" + str(
            int(os.getenv('KUNGFU_TEST_SELF_RANK')))
    print("DBG> Loading checkpoint from: " + ckpt_dir)
    if p.search(ckpt_dir):
        ...
        if ckpt and ckpt.model_checkpoint_path:
            ...
            raise CheckpointNotFoundException(
                'No checkpoint file found in dir:{}'.format(ckpt_dir))
    andrei_print("Restoring checkpoint from " + model_checkpoint_path)
    ...
    for name, value in params._asdict().items():
        ...
        if param_spec.flag_type in ('integer', 'float'):
            if (value is not None
                    and param_spec.kwargs['lower_bound'] is not None
                    and value < param_spec.kwargs['lower_bound']):
                raise ValueError(
                    'Param %s value of %s is lower than the lower bound '
                    'of %s' % (name, value, param_spec.kwargs['lower_bound']))
            if (value is not None
                    and param_spec.kwargs['upper_bound'] is not None
                    and param_spec.kwargs['upper_bound'] < value):
                raise ValueError(
                    'Param %s value of %s is higher than the upper bound '
                    'of %s' % (name, value, param_spec.kwargs['upper_bound']))
        elif (value is not None and param_spec.flag_type == 'enum'
              and value not in param_spec.kwargs['enum_values']):
...
def make_params_from_flags():
  ...
  """
    ...
    # flags.param_specs.
    flag_values = {
        name: getattr(absl_flags.FLAGS, name)
...
def remove_param_fields(params, fields_to_remove):
    ...
    params_dict = {
        k: v
        for k, v in params_dict.items() if k not in fields_to_remove
    }
...
def get_piecewise_learning_rate(piecewise_learning_rate_schedule, global_step,
    ...
    with tf.name_scope('learning_rate'):
        ...
        if params.piecewise_learning_rate_schedule:
            if (params.init_learning_rate is not None
                    or params.learning_rate_decay_factor
                    or params.minimum_learning_rate
                    or params.num_epochs_per_decay):
                raise ValueError(
            ...
            learning_rate = get_piecewise_learning_rate(
                params.piecewise_learning_rate_schedule, global_step,
                num_batches_per_epoch)
        elif params.init_learning_rate is not None:
            learning_rate = params.init_learning_rate
            if (params.num_epochs_per_decay > 0
                    and params.learning_rate_decay_factor > 0):
...
class BenchmarkCNN(object):
    ...
    """
        ...
        self.dataset = dataset or datasets.create_dataset(
            self.params.data_dir, self.params.data_name)
        ...
        if not self.params.eval:
            import os
            if os.getenv('KUNGFU_TEST_SELF_RANK') is None:
                self.filepath, self.version = _checkpoint_path(
                    params.checkpoint_directory, params.checkpoint_version)
            else:
                self.filepath, self.version = _checkpoint_path(
                    params.checkpoint_directory + '-worker-' +
        ...
        if self.params.gpu_indices:
            self.gpu_indices = [
                int(x) for x in self.params.gpu_indices.split(',')
            ]
        else:
            self.gpu_indices = [x for x in range(self.num_gpus)]

        if (self.params.device == 'cpu' and self.params.data_format == 'NCHW'
                and not self.params.mkl):
            raise ValueError('device=cpu requires that data_format=NHWC')

        if ((self.params.num_epochs_per_decay
             or self.params.learning_rate_decay_factor)
                and not (self.params.init_learning_rate is not None
                         and self.params.num_epochs_per_decay
        ...
        if (self.params.minimum_learning_rate
                and not (self.params.init_learning_rate is not None
                         and self.params.num_epochs_per_decay
                         and self.params.learning_rate_decay_factor)):
        ...
        if (self.params.use_fp16 and self.params.fp16_vars
                and 'replicated' in self.params.variable_update
                and self.params.all_reduce_spec
                and 'nccl' in self.params.all_reduce_spec):
            raise ValueError('fp16 variables are not supported with NCCL')
        if (self.params.use_fp16 and self.params.fp16_vars
                and self.params.gradient_repacking):
            raise ValueError(
        ...
        if self.params.use_fp16 and self.params.fp16_enable_auto_loss_scale:
            ...
            if self.params.variable_update not in ('parameter_server',
                                                   'replicated',
                                                   'independent'):
                raise ValueError(
        ...
        if (self.params.debugger is not None and self.params.debugger != 'cli'
                and ':' not in self.params.debugger):
        ...
        eval_during_training_flags = list(
            map(bool, [
        ...
        use_ps_server = self.job_name and (
            ...
            and self.params.variable_update != 'collective_all_reduce')
        ...
        num_train_examples_per_epoch = self.dataset.num_examples_per_epoch(
            'train')
        ...
        if self.params.eval_during_training_at_specified_steps:
            try:
                self.eval_during_training_at_specified_steps = set(
                    map(int,
        ...
        if self.params.eval_during_training_at_specified_epochs:
            try:
                ...
                if offset.is_integer():
                    offset = int(offset)
                mlperf.logger.log(key=mlperf.tags.EVAL_EPOCH_OFFSET,
                                  value=offset)
                self.eval_during_training_at_specified_steps = {
                    (int(e * num_train_examples_per_epoch + self.batch_size -
                         1) // self.batch_size)
                    for e in n_epochs
                }
            except ValueError:
        ...
        #
        print(
            "DBG>", "Train for %d iterations or %d epochs" %
            (self.num_batches, self.num_epochs))
        print(
            "DBG>", "Approximately %d iterations per epoch" %
        ...
        self.checkpoint_interval = int(
            ...
            (self.dataset.num_examples_per_epoch(subset) / self.batch_size))
        print("DBG>",
              "Checkpoint every %d iterations" % self.checkpoint_interval)

        if (self.params.checkpoint_every_n_epochs
                and self.checkpoint_interval == 0):
            raise ValueError("Undefined checkpoint interval")

        if (self.params.staged_vars
                and self.params.variable_update != 'parameter_server'):
        ...
        self.datasets_use_prefetch = (
            ...
            # CPU.
            self.params.device.lower() != 'cpu' and self.input_preprocessor
            and self.input_preprocessor.supports_datasets())
        ...
        if self.params.benchmark_log_dir is not None:
            try:
                ...
                tf.logging.fatal(
                    ...
                    'benchmark_log_dir: %s' % self.params.benchmark_log_dir)
                raise
    ...
    def raw_devices_across_tasks(self, is_local=False):
        ...
        if is_local:
            ...
            return [
                ...
                (t, self.params.device, i) for t in xrange(self.num_workers)
                for i in xrange(self.num_gpus)
            ]
    ...
    def print_info(self):
        ...
        if (self.params.variable_update == 'replicated'
                or self.params.variable_update == 'distributed_all_reduce'
    ...
    """
        ...
        if single_session:
            ...
            device_list = [
                'horovod/%s:%d' % (self.params.device, idx)
    ...
    """
        ...
        if self.params.variable_update == 'distributed_all_reduce':
            if self.params.job_name == 'worker':
                ...
                raise ValueError('unrecognized job name: %s' %
    ...
    def _run_eval_modified(self):
        andrei_print("Inside _run_eval_modified()")
        info = self._build_eval_graph()
        saver = tf.train.Saver(self.variable_mgr.savable_variables())
        writer = tf.summary.FileWriter(self.params.eval_dir,
                                       tf.get_default_graph())
        target = ''
        ...
        for filename in filenames:
            ...
            with tf.Session(target=target,
                ...
                image_producer = self._initialize_eval_graph(
                    info.enqueue_ops, info.input_producer_op,
                    info.local_var_init_op_group, session)

                self._eval_once(session, writer, info.fetches, info.summary_op,
                                image_producer, step)
        ...
        with self._do_eval():
            ...
            with tf.control_dependencies([local_var_init_op]):
                variable_mgr_init_ops.extend(
                    self.variable_mgr.get_post_init_ops())
            ...
            return GraphInfo(input_producer_op, enqueue_ops, fetches,
                             execution_barrier, global_step,
                             local_var_init_op_group, summary_op)
    ...
    def _eval_once(self, sess, summary_writer, fetches, summary_op,
        ...
        with self._do_eval():
            mlperf.logger.log_eval_epoch(mlperf.tags.EVAL_START, global_step,
                                         self.batch_size)
            ...
            print("DBG> %d batches %d images/batch %d images" %
                  (self.num_batches, self.batch_size, total_eval_count))
            ...
            for step in xrange(self.num_batches):
                ...
                if (step + 1) % self.params.display_every == 0:
                    ...
                    examples_per_sec = (self.batch_size *
                                        self.params.display_every / duration)
                    log_fn('%i\t%.1f examples/sec' %
            ...
            summary.value.add(tag='eval/Accuracy@1',
                              simple_value=accuracy_at_1)
            summary.value.add(tag='eval/Accuracy@5',
                              simple_value=accuracy_at_5)
            ...
            log_fn(
                '[Global Step %d] Accuracy @ 1 = %.4f Accuracy @ 5 = %.4f [%d examples]'
                %
                (global_step, accuracy_at_1, accuracy_at_5, total_eval_count))
            # Flush
            sys.stdout.flush()
            elapsed_time = loop_end_time - loop_start_time
            images_per_sec = (self.num_batches * self.batch_size /
                              elapsed_time)
            if self.mode != constants.BenchmarkMode.TRAIN_AND_EVAL:
            ...
            mlperf.logger.log_eval_epoch(mlperf.tags.EVAL_STOP, global_step,
                                         self.batch_size)
            ...
            if self.params.model != 'ssd300':  # ssd300 logs eval accuracy elsewhere.
                mlperf.logger.log_eval_accuracy(
                    ...
                    examples_per_epoch=self.dataset.num_examples_per_epoch(
                        'train'))
    ...
    """
        ...
        if (not self.single_session and self.job_name
                and not self.params.cross_replica_sync):
            execution_barrier = self.add_sync_queues_and_barrier(
                'execution_barrier_', [])

        global_step = tf.train.get_global_step()
        with tf.device(
                self.global_step_device), tf.name_scope('inc_global_step'):
            with tf.control_dependencies([main_fetch_group]):
                fetches['inc_global_step'] = global_step.assign_add(1)

        if ((not self.single_session) and (not self.distributed_collective)
                and self.job_name and self.params.cross_replica_sync):
        ...
        if not self.forward_only_and_freeze:
            with tf.control_dependencies([local_var_init_op]):
                variable_manager_init_ops.extend(
                    self.variable_mgr.get_post_init_ops())
        if ((not self.single_session) and (not self.distributed_collective)
                and self.job_name and self.params.cross_replica_sync):
        ...
        return GraphInfo(input_producer_op=input_producer_op,
    ...
    """
        ...
        summary_writer = None
        if (is_chief and self.params.summary_verbosity
                and self.params.train_dir
        ...
        if ((self.num_warmup_batches + len(graph_info.enqueue_ops) - 1) %
            ...
            self.num_warmup_batches = int(
                math.ceil((self.num_warmup_batches +
                           len(graph_info.enqueue_ops) - 1.0) /
            ...
            assert (
                (self.num_warmup_batches + len(graph_info.enqueue_ops) - 1) %
                self.batch_group_size) == 0
        ...
        if eval_graph_info:
            ...
            with tf.control_dependencies(local_var_init_ops):
                local_var_init_ops.append(
                    eval_graph_info.local_var_init_op_group)
        ...
        target = self.cluster_manager.get_target(
        ) if self.cluster_manager else ''
    ...
    """
        if self.params.backbone_model_path is not None:
            self.model.load_backbone_model(sess,
                                           self.params.backbone_model_path)
        ...
        if eval_graph_info:
            ...
            eval_image_producer = self._initialize_eval_graph(
                eval_graph_info.enqueue_ops,
                eval_graph_info.input_producer_op,
                local_var_init_op_group=None,
                sess=sess)
        step_train_times = []
        ...
        while not done_fn():
            ...
            local_step += 1
            if (self.params.save_model_steps
                    and local_step % self.params.save_model_steps == 0
                    and local_step > 0 and is_chief):
                supervisor.saver.save(sess, supervisor.save_path,
                                      supervisor.global_step)
            if (eval_graph_info and local_step > 0 and not done_fn()
                    and self._should_eval_during_training(local_step)):
                ...
                mlperf.logger.log(key=mlperf.tags.INPUT_SIZE,
                                  value=num_steps_since_last_eval *
                                  self.batch_size)
        ...
        if not global_step_watcher:
            ...
            average_wall_time = (elapsed_time * self.num_workers /
        ...
        if is_chief:
            if self.benchmark_logger:
                self.benchmark_logger.log_metric('average_examples_per_sec',
                                                 images_per_sec,
                                                 global_step=num_steps)
        ...
        success = bool(
            self.model.reached_target()
            or (accuracy_at_1 and self.params.stop_at_top_1_accuracy
                and accuracy_at_1 >= self.params.stop_at_top_1_accuracy))
    ...
    """
        ...
        flattened_op_names = list(
            set([
                v.name.split(':')[0] for v in nest.flatten(graph_info)
                if v is not None
            ]))
        ...
        variables_to_keep = {
            graph_info.global_step: tf.GraphKeys.GLOBAL_VARIABLES
        }
        ...
        if self.params.trt_mode:
            ...
            graphdef = trt.create_inference_graph(
                ...
                max_workspace_size_bytes=self.params.
        ...
        with updated_graph.as_default():
            ...
            for variable in variables_to_keep:
                ...
                tf.add_to_collection(variables_to_keep[variable],
                                     updated_variable)
                if variable is graph_info.global_step:
                    updated_global_step = updated_variable

        updated_graph_info = GraphInfo(
            input_producer_op=_get_tensors_or_ops(
                graph_info.input_producer_op),
            enqueue_ops=_get_tensors_or_ops(graph_info.enqueue_ops),
            execution_barrier=_get_tensors_or_ops(
                graph_info.execution_barrier),
    ...
    """
        ...
        if not self._doing_eval:
            mlperf.logger.log(key=mlperf.tags.INPUT_BATCH_SIZE,
        ...
        with tf.device(self.cpu_device):
            if self._doing_eval:
                ...
                subset = 'train'
            input_list = input_preprocessor.minibatch(self.dataset,
                                                      subset=subset,
                                                      params=self.params,
    ...
    def _maybe_initialize_fp16(self):
        ...
        if self.params.use_fp16 and not self._doing_eval:
            init_loss_scale_val = float(self.params.fp16_loss_scale
                                        or self.model.get_fp16_loss_scale())
            self.loss_scale = None
            self.loss_scale_normal_steps = None
    ...
    def _build_model(self):
        ...
        with tf.name_scope('input_processing'):
            ...
            if input_processing_info.input_producer_op is not None:
                input_producer_op = tf.group(
                    *input_processing_info.input_producer_op)
        update_ops = None
        staging_delta_ops = []
        ...
        if not self.datasets_use_prefetch:
            ...
            enqueue_ops.append(
                tf.group(*gpu_compute_stage_ops,
        ...
        if (self.mode == constants.BenchmarkMode.TRAIN_AND_EVAL
                and self.params.variable_update == 'replicated'):
    ...
    def _build_fetches(self, global_step, all_logits, losses, device_grads,
        ...
        if self.params.compute_lr_on_cpu:
            with tf.device(self.cpu_device):
                learning_rate = get_learning_rate(
                    ...
                    self.dataset.num_examples_per_epoch(), self.model,
                    examples_per_step)
        ...
        for d, device in enumerate(apply_gradient_devices):
            with tf.device(device):
                ...
                loss_scale_params = variable_mgr_util.AutoLossScaleParams(
                    ...
                    inc_loss_scale_every_n=self.params.
        ...
        with tf.device(self.cpu_device):
            if self.task_index == 0 and self.params.summary_verbosity >= 1:
                ...
                tf.summary.scalar(self.params.loss_type_to_report,
                                  average_loss)
    ...
    """
        ...
        for task_num in range(self.num_workers):
            ...
            if input_processing_info.input_producer_op is not None:
                global_input_producer_op.extend(
                    input_processing_info.input_producer_op)
            ...
            for rel_device_num in range(len(self.devices)):
                ...
                with self.variable_mgr.create_outer_variable_scope(
                        abs_device_num), tf.name_scope(
                            'task_%i_tower_%i' %
                            (task_num, rel_device_num)) as name_scope:
        ...
        if gpu_compute_stage_ops:
            enqueue_ops.append(
                tf.group(*gpu_compute_stage_ops, name='gpu_compute_stage_ops'))
        assert not self.variable_mgr.supports_staged_vars()
    ...
    def add_forward_pass_and_gradients(self, phase_train, rel_device_num,
                                       abs_device_num, input_processing_info,
                                       gpu_compute_stage_ops,
                                       gpu_grad_stage_ops):
        ...
        if self.datasets_use_prefetch:
            assert input_processing_info.multi_device_iterator_input, (
                ...
                'datasets_use_prefetch=True')
            input_list = (input_processing_info.
                          multi_device_iterator_input[rel_device_num])
        else:
            if not self.dataset.use_synthetic_gpu_inputs():
        ...
        labels_device_placement_hack = (
            self.dataset.use_synthetic_gpu_inputs()
            and self.params.xla_compile)
      ...
      """
            ...
            base_loss = self.model.loss_function(input_list,
            ...
            with tf.name_scope('l2_loss'):
                ...
                if rel_device_num == len(self.devices) - 1:
                    ...
                    if self.params.single_l2_loss_op:
                        ...
                        # tensors.
                        reshaped_params = [
                            tf.reshape(p, (-1, )) for p in filtered_params
                        ]
                        l2_loss = tf.nn.l2_loss(
                            tf.concat(reshaped_params, axis=0))
                    else:
                        l2_loss = tf.add_n(
                            [tf.nn.l2_loss(v) for v in filtered_params])
            weight_decay = self.params.weight_decay
            mlperf.logger.log(key=mlperf.tags.OPT_WEIGHT_DECAY,
                              value=weight_decay)
            if (weight_decay is not None and weight_decay != 0.
                    and l2_loss is not None):
                mlperf.logger.log(key=mlperf.tags.MODEL_L2_REGULARIZATION,
                                  value=weight_decay)
                total_loss += len(self.devices) * weight_decay * l2_loss

            aggmeth = tf.AggregationMethod.DEFAULT
            scaled_loss = (total_loss if self.loss_scale is None else
                           total_loss * self.loss_scale)
            grads = tf.gradients(scaled_loss,
                                 params,
            ...
            if self.loss_scale is not None:
                ...
                grads = [
                    grad * tf.cast(1. / self.loss_scale, grad.dtype)
                    for grad in grads
                ]
        ...
        def unpack_forward_pass_and_gradients_output(
      ...
      """
            ...
            loss = (forward_pass_and_grad_outputs[0]
                    if forward_pass_and_grad_outputs else None)
            grads = (forward_pass_and_grad_outputs[1:]
                     if forward_pass_and_grad_outputs else None)
        ...
        with tf.device(self.devices[rel_device_num]):
            ...
            logits, loss, grads = unpack_forward_pass_and_gradients_output(
                outputs)
    ...
    """
        ...
        with tf.device(
                self.sync_queue_devices[(self.sync_queue_counter %
                                         len(self.sync_queue_devices))]):
            sync_queues = [
                tf.FIFOQueue(self.num_workers, [tf.bool],
                             shapes=[[]],
            ...
            queue_ops.append(sync_queues[self.task_index].dequeue_many(
                len(sync_queues) - 1))
...
def _set_environ_vars(params):
    ...
    if params.mkl:
        mkl_flags = [
            'kmp_blocktime', 'kmp_settings', 'kmp_affinity',
            'num_intra_threads'
        ]
    ...
    if params.device.lower() == 'gpu':
        params = params._replace(
            gpu_thread_mode=params.gpu_thread_mode.lower())
        if params.gpu_thread_mode not in [
                'global', 'gpu_shared', 'gpu_private'
        ]:
            raise ValueError('Invalid gpu_thread_mode: %s' %
                             params.gpu_thread_mode)
        ...
        if (params.datasets_use_prefetch
                and params.datasets_num_private_threads is None):
            ...
            params = params._replace(
                datasets_num_private_threads=num_private_threads)
    return params

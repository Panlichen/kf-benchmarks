...
# TensorFlow 2.x 不再支持 `tf.contrib`，因此删除这行
...
# 禁用 Eager Execution 以保持 TensorFlow 1.x 的 graph 模式
...
def create_config_proto(params):
    ...
    if params.gpu_memory_frac_for_testing > 0:
        config.gpu_options.per_process_gpu_memory_fraction = params.gpu_memory_frac_for_testing
...
def get_learning_rate(params, global_step, num_examples_per_epoch, model,
    ...
    with tf.name_scope('learning_rate'):
        ...
        if params.num_learning_rate_warmup_epochs > 0 and (
            ...
            learning_rate = tf.compat.v1.cond(global_step < warmup_steps,
...
def generate_tfprof_profile(profiler, tfprof_file):
    ...
    with tf.io.gfile.GFile(tfprof_file, 'wb') as f:
...
class BenchmarkCNN(object):
    ...
    def __init__(self, params, dataset=None, model=None):
        ...
        """
        self.params = params
        self._doing_eval = params.eval

        # 创建数据集和模型
        self.dataset = dataset or datasets.create_dataset(self.params.data_dir, self.params.data_name)
        self.model = model or model_config.get_model_config(self.params.model, self.dataset, self.params)
        
        self.trace_filename = self.params.trace_file
        self.rewriter_config = self.params.rewriter_config
        autotune_threshold = self.params.autotune_threshold if self.params.autotune_threshold else 1
        min_autotune_warmup = 5 * autotune_threshold * autotune_threshold
        self.num_warmup_batches = self.params.num_warmup_batches if self.params.num_warmup_batches is not None else max(10, min_autotune_warmup)
        ...
        if self.params.device == 'cpu' and self.params.data_format == 'NCHW' and not self.params.mkl:
            raise ValueError('device=cpu requires that data_format=NHWC')

        if ((self.params.num_epochs_per_decay or self.params.learning_rate_decay_factor) and
            not (self.params.init_learning_rate is not None and self.params.num_epochs_per_decay and self.params.learning_rate_decay_factor)):
            raise ValueError('If one of num_epochs_per_decay or learning_rate_decay_factor is set, both must be set and learning_rate must be set')

        if (self.params.minimum_learning_rate and
            not (self.params.init_learning_rate is not None and self.params.num_epochs_per_decay and self.params.learning_rate_decay_factor)):
            raise ValueError('minimum_learning_rate requires learning_rate, num_epochs_per_decay, and learning_rate_decay_factor to be set')
        ...
        if self.params.use_fp16 and self.params.fp16_vars and self.params.gradient_repacking:
        ...
        if self.params.use_fp16 and self.params.fp16_enable_auto_loss_scale:
            ...
            if self.params.variable_update not in ('parameter_server', 'replicated', 'independent'):
                raise ValueError('Automatic loss scaling is not supported with variable_update=%s.' % self.params.variable_update)
            if self.params.staged_vars:
                raise ValueError('Automatic loss scaling is not supported with staged_vars.')

        if self.params.debugger is not None and self.params.debugger != 'cli' and ':' not in self.params.debugger:
            raise ValueError('--debugger must be "cli" or in the form host:port')

        if self.params.hierarchical_copy and self.params.num_gpus <= 1:
            raise ValueError('--hierarchical_copy requires --num_gpus to be greater than 1')

        if params.save_model_secs and params.save_model_steps:
            raise ValueError('At most one of --save_model_secs and --save_model_steps can be specified')
        ...
        if eval_during_training_flags.count(True) > 1:
            raise ValueError('At most one flag with --eval_during_training_* prefix must be specified.')

        eval_during_training_enabled = any(eval_during_training_flags)

        if eval_during_training_enabled:
            if params.eval:
                raise ValueError('At most one of --eval and --eval_during_training_* must be specified')
            if params.forward_only:
                raise ValueError('At most one of --forward_only and --eval_during_training_* must be specified')
            if params.job_name:
                raise ValueError('--eval_during_training_* is not yet supported in distributed mode.')
            if params.staged_vars:
                raise ValueError('--eval_during_training_* is not currently compatible with staged_vars')

        if params.stop_at_top_1_accuracy and not eval_during_training_enabled:
            raise ValueError('--stop_at_top_1_accuracy is only supported with --eval_during_training_*')

        if self.params.forward_only and self.params.freeze_when_forward_only:
            if self.params.train_dir is not None:
                raise ValueError('In forward_only mode, when --freeze_when_forward_only is True, --train_dir should not be specified')
            if self.params.data_dir and not self.params.datasets_use_prefetch:
                raise ValueError('In forward_only mode, when --freeze_when_forward_only is True and --data_dir is set, --datasets_use_prefetch should be set to True')
            if self.params.job_name:
                raise ValueError('In forward_only mode, when --freeze_when_forward_only is True, --job_name should not be specified and distributed running is not supported')
            self.forward_only_and_freeze = True
        else:
            self.forward_only_and_freeze = False
            if self.params.trt_mode:
                raise ValueError('--trt_mode should not be specified if one of --forward_only and --freeze_when_forward_only is set to False')

        # 使用命令行指定的 batch size，否则使用模型的默认 batch size
        ...
        self.enable_auto_loss_scale = self.params.use_fp16 and self.params.fp16_enable_auto_loss_scale
        ...
        use_ps_server = self.job_name and (self.params.variable_update != 'distributed_all_reduce' and
                                           self.params.variable_update != 'collective_all_reduce')
        # controller is used for distributed_all_reduce with > 1 worker.
        use_controller = (self.params.variable_update == 'distributed_all_reduce' and self.job_name)
        if use_controller and not params.controller_host:
            raise ValueError('When variable_update==distributed_all_reduce controller_host must also be specified.')

        # collective_all_reduce doesn't need a controller or ps
        self.distributed_collective = (self.params.variable_update == 'collective_all_reduce' and self.job_name)

        self.local_parameter_device_flag = self.params.local_parameter_device
        if self.job_name:
            self.task_index = self.params.task_index
            self.cluster_manager = platforms_util.get_cluster_manager(params, create_config_proto(params))
        ...
        if self.num_workers > 1 and self.params.all_reduce_spec == 'nccl':
            raise ValueError('--all_reduce_spec=nccl is invalid in a multi-worker job')
        ...
        if self.params.eval_during_training_at_specified_epochs:
            try:
                n_epochs = map(float, self.params.eval_during_training_at_specified_epochs)
                self.eval_during_training_at_specified_steps = {
                    (int(e * num_train_examples_per_epoch + self.batch_size - 1) //
                     self.batch_size)
                    for e in n_epochs}
        ...
        if self.params.staged_vars and self.params.variable_update != 'parameter_server':
        ...
        if self.params.variable_update == 'parameter_server':
            if self.job_name:
                if not self.params.staged_vars:
                    ...
                    self.variable_mgr = variable_mgr.VariableMgrDistributedFetchFromStagedPS(self)
            else:
                if not self.params.staged_vars:
            ...
            if self.job_name:
                raise ValueError('Invalid variable_update in distributed mode: %s' % self.params.variable_update)
            ...
            if not self.job_name:
                raise ValueError('Invalid variable_update in local mode: %s' % self.params.variable_update)
            ...
            raise ValueError('Invalid variable_update: %s' % self.params.variable_update)
        ...
        if not self.dataset.use_synthetic_gpu_inputs():
            ...
            if self.mode in (constants.BenchmarkMode.EVAL, constants.BenchmarkMode.TRAIN_AND_EVAL):
                with self._do_eval():
    ...
    def print_info(self):
        ...
        if self.batch_group_size > 1:
            log_fn('             %d batches per preprocessing group' %
    ...
    def _build_eval_graph(self, scope_name=None):
        ...
        with self._do_eval():
            ...
            with tf.compat.v1.control_dependencies([local_var_init_op]):
        ...
        with self._do_eval():
            ...
            for step in range(self.num_batches):
                ...
                if (step + 1) % self.params.display_every == 0:
                    ...
                    examples_per_sec = (self.batch_size * self.params.display_every / duration)
            ...
            for result_key, result_value in results.items():
                if result_key.startswith(constants.SIMPLE_VALUE_RESULT_PREFIX):
                    ...
                    summary.value.add(tag='eval/' + result_key[prefix_len:], simple_value=result_value)
            if summary_writer:
    ...
    def _benchmark_graph(self, graph_info, eval_graph_info):
        ...
        # We want to start the benchmark timer right after an image_producer barrier
        # and avoid undesired waiting times on barriers.
        ...
        if self.job_name and not (self.single_session or self.distributed_collective):
            ...
            # in replicated mode).
            ready_for_local_init_op = tf.compat.v1.report_uninitialized_variables(tf.compat.v1.global_variables())
        ...
        if self.params.variable_update == 'horovod':
            ...
            ops = [tf.compat.v1.assign(v, broadcast(v)) for v in tf.compat.v1.global_variables()]
            bcast_global_variables_op = tf.compat.v1.group(ops)
        else:
        ...
        if eval_graph_info:
            ...
            with tf.compat.v1.control_dependencies(local_var_init_ops):
    ...
    def benchmark_with_session(self, sess, supervisor, graph_info,
        ...
        bcast_global_variables_op: If Horovod is used, the op to broadcast the
            ...
            will initialize global variables on all processes based on the required
        ...
        if self.params.train_dir is not None and is_chief:
            ...
            if not tf.io.gfile.exists(self.params.train_dir):
                tf.io.gfile.makedirs(self.params.train_dir)
    ...
    def _preprocess_graph(self, graph, graph_info):
        ...
        assert isinstance(graph_info.global_step, tf.compat.v1.Variable)
        if not self.forward_only_and_freeze:
        ...
        if self.params.trt_mode:
            ...
            name_to_input_name, _, _ = tf.compat.v1.graph_util_impl._extract_graph_summary(
                graphdef)
            initializer_subgraph_ops = tf.compat.v1.graph_util_impl._bfs_for_reachable_nodes(
                variable_initializers, name_to_input_name)
            # pylint: enable=protected-access

            converter = trt.TrtGraphConverter(
                input_graph_def=graphdef,
                nodes_blacklist=output_node_names + list(initializer_subgraph_ops),
        ...
        with updated_graph.as_default():
            tf.compat.v1.import_graph_def(graph_def=graphdef, name='')

            # Update the variables
            for variable in variables_to_keep:
                updated_variable = tf.compat.v1.Variable.from_proto(variable.to_proto())
    ...
    def _build_input_processing(self, shift_ratio=0):
        ...
        with tf.compat.v1.device(self.cpu_device):
            ...
            for device_num in range(len(self.devices)):
                staging_area = tf.compat.v1.data.experimental.StagingArea(
    ...
    def _build_model(self):
        ...
        # Adjust seed so different workers start reading different input files.
        if self.params.variable_update == 'horovod':
            import horovod.tensorflow as hvd  # pylint: disable=g-import-not-at-top
        ...
        with tf.name_scope('input_processing'):
            ...
            if input_processing_info.input_producer_op is not None:
                input_producer_op = tf.compat.v1.group(*input_processing_info.input_producer_op)
        update_ops = None
        staging_delta_ops = []
        ...
        if not self.datasets_use_prefetch:
            ...
            enqueue_ops.append(tf.compat.v1.group(*gpu_compute_stage_ops,
                                                name='gpu_compute_stage_ops_group'))
            if gpu_grad_stage_ops:
                staging_delta_ops += gpu_grad_stage_ops
            if staging_delta_ops:
                enqueue_ops.append(tf.compat.v1.group(*(staging_delta_ops)))
            ...
            with tf.compat.v1.device(device):
                ...
                with tf.name_scope('append_apply_gradient_ops'):
                    self.variable_mgr.append_apply_gradients_ops(
                        ...
                        loss_scale_params)
        train_op = tf.compat.v1.group(*(training_ops + update_ops), name='train_ops_group')
    ...
    def _build_model_single_session(self):
        ...
        if global_input_producer_op:
            global_input_producer_op = tf.compat.v1.group(*global_input_producer_op)
        else:
    ...
    def add_forward_pass_and_gradients(self,
        ...
        if self.datasets_use_prefetch:
            ...
            with tf.compat.v1.device(self.raw_devices[rel_device_num]):
                if function_buffering_resource is not None:
                    subset = 'validation' if self._doing_eval else 'train'
                    input_list = tf.raw_ops.FunctionBufferingResourceGetNext(
                        function_buffering_resource=function_buffering_resource,
            ...
            if not self.dataset.use_synthetic_gpu_inputs():
                ...
                with tf.compat.v1.device(self.raw_devices[rel_device_num]):
                    gpu_compute_stage = tf.compat.v1.data.experimental.StagingArea(
        ...
        def forward_pass_and_gradients():
            ...
            aggmeth = tf.compat.v1.AggregationMethod.DEFAULT
            scaled_loss = (total_loss if self.loss_scale is None
                        else total_loss * self.loss_scale)
            grads = tf.compat.v1.gradients(scaled_loss, params, aggregation_method=aggmeth)
            ...
            if self.params.staged_vars:
                ...
                grad_stage = tf.compat.v1.data.experimental.StagingArea(grad_dtypes, grad_shapes)
    ...
    def add_sync_queues_and_barrier(self, name_prefix, enqueue_after_list):
        ...
        with tf.compat.v1.device(self.sync_queue_devices[(
            ...
            sync_queues = [
                tf.compat.v1.FIFOQueue(self.num_workers, [tf.bool], shapes=[[]],
...
def maybe_compile(computation, params):
    if params and params.xla_compile:
        return tf.compat.v1.xla.experimental.compile(computation)
    else:
        return computation()

...
class ConvNetBuilder(object):
    ...
    def __init__(self,
        ...
        print(f"shapes: self.top_layer: {self.top_layer.get_shape().as_list()}")
    ...
    def get_custom_getter(self):
        """返回一个自定义getter，必须在该getter下调用此类的方法。

        此类的所有方法必须在传递了此自定义getter的变量作用域中调用。例如：
        ...
        目前，如果self.use_tf_layers为True，则此自定义getter才有作用。
        在这种情况下，它会使变量存储为self.variable_type类型，然后转换为请求的dtype，而不是直接以请求的dtype存储变量。
        """
        def inner_custom_getter(getter, *args, **kwargs):
            """自定义getter，强制变量的类型为self.variable_type。"""
            if not self.use_tf_layers:
                return getter(*args, **kwargs)
            requested_dtype = kwargs['dtype']
            if not (requested_dtype == tf.float32 and
                    self.variable_dtype == tf.float16):
                # 仅当更改变量的dtype不会降低变量的精度时才更改变量的dtype。
                kwargs['dtype'] = self.variable_dtype
            var = getter(*args, **kwargs)
            # 这个if语句是为了保护cast，因为batch norm直接分配给这个自定义getter的返回值。
            # cast使得返回值不是变量，因此不能被分配。Batch norm变量始终为fp32，因此这个if语句对它们永远不会触发。
    ...
    def get_variable(self, name, shape, dtype, cast_dtype, *args, **kwargs):
        var = tf.compat.v1.get_variable(name, shape, dtype=dtype, *args, **kwargs)
    ...
    def _conv2d_impl(self, input_layer, num_channels_in, filters, kernel_size,
        ...
        if self.use_tf_layers:
            return keras_layers.Conv2D(filters, kernel_size, strides,
                                       padding, data_format=self.channel_pos,
    ...
    def _pool(self,
        ...
        pool = pool_function(
            [k_height, k_width], [d_height, d_width],
            padding=mode,
    ...
    def mpool(self,
              ...
              num_channels_in=None):
        """构造一个最大池化层。"""
        return self._pool('mpool', keras_layers.MaxPooling2D, k_height, k_width,
    ...
    def apool(self,
              ...
              num_channels_in=None):
        """构造一个平均池化层。"""
        return self._pool('apool', keras_layers.AvgPool2D, k_height,
    ...
    def reshape(self, shape, input_layer=None):
        ...
        self.top_size = shape[-1]  # HACK 这可能并不总是有效
    ...
    #                                self.variable_dtype, self.dtype,
    #                                initializer=tf.constant_initializer(bias))
    #     logits = tf.linalg.matmul(input_layer, kernel) + biases
    ...
    def affine(self,
      ...
      with tf.compat.v1.variable_scope(name):
          ...
          biases = self.get_variable('biases', [num_out_channels],
                                    self.variable_dtype, self.dtype,
                                    initializer=tf.constant_initializer(bias))
          print(f"shapes: input_layer: {input_layer.get_shape().as_list()}, kernel: {kernel.get_shape().as_list()}, biases: {biases.get_shape().as_list()}")
          logits = tf.linalg.matmul(input_layer, kernel) + biases
          if activation == 'relu':
              affine1 = tf.nn.relu(logits, name=name)
          elif activation == 'linear' or activation is None:
    ...
    def spatial_mean(self, keep_dims=False):
        ...
        print(f"shape: self.top_layer 0000: {self.top_layer.get_shape().as_list()}")
        self.top_layer = tf.reduce_mean(
            self.top_layer, axes, keepdims=keep_dims, name=name)
        # self.top_layer = tf.reshape(self.top_layer, [-1, self.top_layer.get_shape().as_list()[-1]])
        print(f"shape: self.top_layer 1111: {self.top_layer.get_shape().as_list()}")
        return self.top_layer
    ...
    def _batch_norm_without_layers(self, input_layer, decay, use_scale, epsilon):
        ...
        moving_mean = tf.Variable(tf.zeros([num_channels]), trainable=False)
        moving_variance = tf.Variable(tf.ones([num_channels]), trainable=False)

        if self.phase_train:
            batch_mean, batch_variance = tf.nn.moments(input_layer, [0, 1, 2] if self.data_format == 'NHWC' else [0, 2, 3])
            ...
            batch_variance = moving_variance

        bn = tf.nn.batch_normalization(input_layer, batch_mean, batch_variance, beta, gamma, epsilon)
        return bn
        ...
        with tf.name_scope(name):
            if self.use_tf_layers:
                bn = keras_layers.BatchNormalization(
                    ...
                    center=True,
                    trainable=self.phase_train)(input_layer)
            else:
